...
const BiomarkerEvaluationRequest = () => {
      ...
      messages.current.show([
        {
          ...
          detail: `Your analysis ID is ${res.data.analysis_id}. You will receive an email notification once your request is processed. You can review the results at https://predictio.ca/explore/biomarker/result/${res.data.analysis_id}`,
          sticky: true,
        },
      ]);
    }
  ...
  const buildQueryStr = (paramList, paramName) => {
    ...
    paramList.forEach((item, i) => {
      console.log(item);
      str = str.concat(
        `${paramName}=${item}${i < paramList.length - 1 ? "&" : ""}`
      );
    });
    return str;
  };
  ...
  useEffect(() => {
    const getDownstreamOptions = async () => {
      ...
      if (parameters.gene.length > 0) {
        ...
        let paramStr = buildQueryStr(
          parameters.gene.map((g) => g.value),
          "gene"
        );
  ...
  const getDownstreamDrugType = async (drugType) => {
    if (parameters.drugType.length > 0) {
      ...
      paramStr = paramStr.concat(
        `&${buildQueryStr(parameters.primary, "primary")}`
      );
  ...
  const getDownstreamSequencing = async (sequencingType) => {
    ...
    if (parameters.sequencingType.length > 0) {
      ...
      paramStr = paramStr.concat(
        `&${buildQueryStr(parameters.drugType, "drugtype")}`
      );
      paramStr = paramStr.concat(
        `&${buildQueryStr(sequencingType, "sequencing")}`
      );
      // get available options and enable downstream dropdowns
        ...
        <StyledForm flexDirection="column">
          <div className="formField">
            ...
            <CustomDropdown
              ...
              onChange={(e) => {
                setParameters({ ...parameters, dataType: e.value });
              }}
              filter={true}
              checkbox={true}
              placeholder="Select..."
          ...
          <div className="formField">
            ...
            <CustomMultiSelect
              ...
              onChange={(e) => {
                getDownstreamSequencing(e.value);
              }}
              filter={true}
              checkbox={true}
              placeholder="Select..."
          ...
          <div className="formField">
            ...
            <CustomMultiSelect
              ...
              onChange={(e) => {
                setParameters({ ...parameters, study: e.value });
              }}
              filter={true}
              checkbox={true}
              placeholder="Select..."
          ...
          <div className="formField buttonField">
            <ActionButton
              onClick={(e) => {
                setParameters({ ...parameters, dataType: "", email: "" });
              }}
              text="Reset"
              type="reset"
              style={{
